...
public class ScheduleTask {
	...
	public void dmpInquiries() {
		 ...
		 * cn.clickwise.rpc.Connection conrpc = new
		 * cn.clickwise.rpc.Connection(); conrpc.setHost(dmps[i].getHost());
		 ...
		 * hftcmd.setKeyFieldName(dmps[i].getUidFieldName()); //String
		 * recordFile = confFactory.getRecordFilePrefix() +
		 * day+"_"+dmps[i].getArea().getAreaCode() + ".txt"; String
		 * recordFile=confFactory.getDmpRecordFile(day, dmps[i]);
		 * hftcmd.setResultName(recordFile);
		 * hftcmd.setResultPath(confFactory.getRecordFileDirectory()+
		 * recordFile);
		 * HiveFetchTableClient.initRandomFileName(confFactory.getTmpIdentify(),
		 * day, hftcmd); hftcmd.setQueryType(confFactory.getQueryType());
		 * 
		 * hftc.connect(conrpc); hftc.execute(hftcmd);
		 * 
		 * rdi.writeRecFile2DataStore(new
		 * File(confFactory.getRecordFileDirectory() + recordFile), con,
		 * dmps[i],day);
		 * 
		 * }
		 */
		...
		for (int i = 0; i < dmps.length; i++) {
			try {
				File dmpRecFile = new File(confFactory.getRecordFileDirectory()
						+ confFactory.getDmpRecordFile(day, dmps[i]));
				if (!(dmpRecFile.exists())) {
					continue;
				}
				// rdi.writeRecFile2DataStore(new
				// File(confFactory.getRecordFileDirectory() +
				// confFactory.getDmpRecordFile(day, dmps[i])), con,
				// dmps[i],day);//用户特征数据写入cassandra
				System.out.println("i=" + i + " "
						+ dmpRecFile.getAbsolutePath());
