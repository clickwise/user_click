...
public class HiveSql {
	...
	{
		String sql="";
		sql=" use clickwise; CREATE TABLE IF NOT EXISTS "+hfkc.getKeyTableName()+"("+hfkc.getKeyFieldName()+" string) PARTITIONED BY(dt STRING,dp string) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\001' LINES TERMINATED BY '\n';";
		return hive+"\""+sql+"\"";
	}
	...
	{
		String sql="";
		sql="use clickwise; INSERT OVERWRITE LOCAL DIRECTORY '"+hfkc.getResultRemotePath()+"' SELECT *  FROM "+hfkc.getTableName()+" a JOIN "+hfkc.getKeyTableName()+" b  ON  a."+hfkc.getKeyFieldName()+"=b."+hfkc.getKeyFieldName()+" where a.dt="+hfkc.getDay()+";";
		return hive+"\""+sql+"\"";
	}
	...
	{
		String cmd="";
		cmd="use clickwise;ALTER TABLE "+hfkc.getKeyTableName()+"  DROP PARTITION (dt='"+hfkc.getDay()+"');";
		return hive+"\""+cmd+"\"";
	}
	...
	{
		String cmd="";
		cmd="use clickwise; load data inpath '"+hfkc.getHdfTmpPath()+"' overwrite into table "+hfkc.getKeyTableName()+" partition(dt="+hfkc.getDay()+",dp='part1');";
		return hive+"\""+cmd+"\"";
	}
