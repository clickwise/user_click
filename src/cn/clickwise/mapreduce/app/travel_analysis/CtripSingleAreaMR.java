...
public class CtripSingleAreaMR {
	...
	private static class PrepareMapper extends Mapper<Object, Text, Text, Text> {
        	...
        	if(seg_arr.length==11)
        	{
        		...
        		if(SSO.tnoe(host))
        		{
                    ...
                    if(SSO.tnoe(Code_A)&&SSO.tnoe(adate_str)&&SSO.tnoe(atime_str)&&SSO.tnoe(area))
                    {
                    	...
                    	word.set(area);
                    	word1.set(adate_str+"\001"+atime_str+"\001"+sip+"\001"+dip+"\001"+Code_A+"\001"+cookie+"\001"+loc+"\001"+agent+"\001"+date_a+"\001ctrip");
		...
		{
        	Pattern ac_pat=Pattern.compile("vacations.ctrip.com/dest/[pPdDcC]\\-([a-zA-Z]*)\\-[\\d]+");
        	Matcher ac_mat=null;
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: CtripSingleAreaMR <day> <input> <output>");
			System.exit(2);
		}
		
		String day = otherArgs[0];
		Job job = new Job(conf, "CtripSingleAreaMR_" + day);
		job.setJarByClass(CtripSingleAreaMR.class);
