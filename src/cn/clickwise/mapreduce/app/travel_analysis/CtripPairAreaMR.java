package cn.clickwise.mapreduce.app.travel_analysis;
...
public class CtripPairAreaMR {
	...
	private static class PrepareMapper extends Mapper<Object, Text, Text, Text> {
		...
		public void map(Object key, Text value, Context context)
        	...
        	if(seg_arr.length==11)
        	{
        		...
        		if(SSO.tnoe(host))
        		{
                    ...
                    if(SSO.tnoe(Code_A)&&SSO.tnoe(Code_B)&&SSO.tnoe(adate_str)&&SSO.tnoe(atime_str)&&SSO.tnoe(area))
                    {
                    	area=area.trim();
                    	adate_str=adate_str.trim();
                    	atime_str=atime_str.trim();
                    	Code_A=Code_A.toUpperCase();
                    	Code_B=Code_B.toUpperCase();
                    	Code_A=Code_A.trim();
                    	Code_B=Code_B.trim();                  	
                    	date_arr=getDADateFromLink(link);
                    	word.set(area);
                    	word1.set(adate_str+"\001"+atime_str+"\001"+sip+"\001"+dip+"\001"+Code_A+"\001"+Code_B+"\001"+cookie+"\001"+loc+"\001"+agent+"\001"+date_arr[0]+"\001"+date_arr[1]+"\001ctrip");
		...
		public String[] getDAAreaFromLink(String link)
		{
			...
			Pattern ac_pat=Pattern.compile("flights.ctrip.com/booking/([A-Z]*)-([A-Z]*)");
        	Matcher ac_mat=null;  

			ac_mat=ac_pat.matcher(link);
					
        	Pattern ac_pat2=Pattern.compile("flights.ctrip.com.*?(?:(?:acity)|(?:dcity)|(?:acity1)|(?:dcity1)|(?:acitycode)|(?:dcitycode))\\s*(?:(?:=)|(?:%3d))\\s*([a-z]*)\\s*(?:(?:&)|(?:%26))\\s*(?:(?:acity)|(?:dcity)|(?:acity1)|(?:dcity1)|(?:acitycode)|(?:dcitycode))\\s*(?:(?:=)|(?:%3d))\\s*([a-z]*)");
		...
		{
        	Pattern ac_pat=Pattern.compile("flights.ctrip.com.*?(?:(?:GoDate)|(?:BackDate))\\s*(?:(?:=)|(?:%3d))\\s*([\\-0-9]*)\\s*(?:(?:&)|(?:%26))\\s*(?:(?:GoDate)|(?:BackDate))\\s*(?:(?:=)|(?:%3d))\\s*([\\-0-9]*)");
        	Matcher ac_mat=null;
			ac_mat=ac_pat.matcher(link);
			...
			date_arr[0]=date_arr[0].toUpperCase();
			date_arr[1]=date_arr[1].toUpperCase();
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: CtripPairAreaMR <day> <input> <output>");
			System.exit(2);
		}
		
		String day = otherArgs[0];
		Job job = new Job(conf, "CtripPairAreaMR_" + day);
		job.setJarByClass(CtripPairAreaMR.class);
