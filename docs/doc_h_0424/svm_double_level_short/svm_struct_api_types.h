...
//存储一个文档，以向量形式存储，例如单词向量，doc->fvector 每个word 的wnum是单词词典索引，weight是该单词在文档中出现的次数
...
typedef struct label {
  ...
  int first_class;       /* 第一级类别 */
  int second_class;      /* 第二级类别 */
//  int third_class;       /* 第三级类别 */
  int num_first_classes; /* 第一级不同类别个数 */
  int num_second_classes; /* 第二级不同类别个数*/
//  int num_third_classes;  /* 第三级不同类别个数 */
  //int num_classes; /* total number of classes */
  //如果存储num_first_classes*num_second_classes*num_third_classes个score太占空间
...
typedef struct struct_learn_parm {
  ...
  int first_num_classes;
  int second_num_classes;
 // int third_num_classes;
  int num_features;
  long top_features;
} STRUCT_LEARN_PARM;
