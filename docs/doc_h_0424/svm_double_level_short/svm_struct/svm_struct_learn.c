			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  */
  ...
  if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
    printf("before create_constraint_cache \n");
    ccache=create_constraint_cache(sample,sparm,sm);
    printf("after create_constraint_cache \n");
    printf("the n is %d \n",n);
    /* NOTE:  */
    for(i=0;i<n;i++) 
...
//  int batch_sel=0;
  do { /* iteratively find and add constraints to working set */
	...
	if(struct_verbosity>=2) rt_cacheupdate+=MAX(get_runtime()-rt2,0);
        printf("epsilon_est is %f \n",epsilon_est);
	/* Is there is a sufficiently violated constraint in cache? */
       // printf("before compute_violation_of_constraint_in_cache");

	viol=compute_violation_of_constraint_in_cache(ccache,epsilon_est/2);
       // printf("after compute_violation_of_constraint_in_cache");
        printf("viol is %lf and slack is %lf  sparm->epsilon is %lf \n",viol,slack,sparm->epsilon);
	...
	to the most violated constraint for example ex */
{
  ...
  if(struct_verbosity>=2) rt2=get_runtime();
  (*argmax_count)++;
 // printf("the loss_type is %d \n",sparm->loss_type);
 
  if(sparm->loss_type == SLACK_RESCALING) 
  {
   // printf("the loss type is SLACK_RESCALING \n");
    exit(0);
    ybar=find_most_violated_constraint_slackrescaling(ex->x,ex->y,sm,sparm);
  }
  else
  {
  //  printf("the loss type is other \n");
  //  exit(0);
...
//  printf("in update_constraint_cache_for_model \n");
  ...
  //  printf("there i is %d and the voil is %lf \n",i,ccache->constlist[i]->viol);
  }
  free_example(doc_fydelta,0);
}
	...
	efficient when loss(y,y)=0. */
{
  ...
  for(i=0; i<n; i++) { 
   // printf("n=%d i=%d and viol is %lf \n",n,i,ccache->constlist[i]->viol);
