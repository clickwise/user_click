...
int main (int argc, char* argv[])
{
  ...
  char *ptlf_file="../ptlf.txt";
  FILE *p_t_l_f;
  if ((p_t_l_f = fopen (ptlf_file, "w")) == NULL)
     { perror (ptlf_file); exit (1); } 
  ...
  for(i=0;i<testsample.n;i++) {
    ...
    /*
    WORD *ai;
    ai=testsample.examples[i].x->doc->fvec->words;
    printf("the words read example:\n");
    while(ai->wnum){
       ai++;
    ...
    */
    ...
    //printf("i= %d y.first_class %d y.second_class %d   \n",i,y.first_class,y.second_class);
   // printf("i= %d e.first_class %d e.second_class %d   \n",i,testsample.examples[i].y.first_class,testsample.examples[i].y.second_class);
 
    fprintf(p_t_l_f,"%d  %d  ",y.first_class,y.second_class);
    fprintf(p_t_l_f,"%d  %d  \n",testsample.examples[i].y.first_class,testsample.examples[i].y.second_class);
    printf("%d  %d  ",y.first_class,y.second_class);
    printf("%d  %d  \n",testsample.examples[i].y.first_class,testsample.examples[i].y.second_class);
...
//	printf("%ld..",i+1); fflush(stdout);
  ...
  float first_loss=((float)first_incorrect)/((float)first_correct+(float)first_incorrect);
  printf("first loss is %.2f \n",first_loss);
