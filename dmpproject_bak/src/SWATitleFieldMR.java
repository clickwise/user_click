...
public abstract class SWATitleFieldMR {
	...
	private static class PrepareMapper extends Mapper<Object, Text, Text, Text> {
		...
		public void map(Object key, Text value, Context context)
			...
			if (seg_arr != null && seg_arr.length == 5) {
                ...
                title_seg=seg_arr[2].trim();
                sws=seg_arr[3].trim();
                ips=seg_arr[4].trim();             
				if ((title_seg!= null) && (!title_seg.equals(""))) {
						word.set(title_seg);
						word1.set("");
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: SWATitleFieldMR <day> <input> <output>");
			System.exit(2);
		}
		
		String day = otherArgs[0];
		Job job = new Job(conf, "SWATitleFieldMR_" + day);
		job.setJarByClass(SWATitleFieldMR.class);
