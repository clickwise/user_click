...
public class SWASegDict {
	...
	public void load_local_config(Properties prop) throws Exception
	{
		seg_server = prop.getProperty("seg_server");
		seg_port = Integer.parseInt(prop.getProperty("seg_port"));
		tag_server = prop.getProperty("tag_server");
		...
		swa_dict_redis.ping();
		swa_dict_redis.select(swa_dict_db);
		sw_word_dir=prop.getProperty("sw_word_dir");
		load_dict_to_redis(sw_word_dir);
	 ...
	 * @throws Exception
	 */
	public int load_dict_to_redis(String wci_dir) throws Exception {
		int res = 0;
		File wci_dir_file = new File(wci_dir);
		File[] wci_files = wci_dir_file.listFiles();
		long rdb_size = 0;
		rdb_size = swa_dict_redis.dbSize();
		if (rdb_size > 0) {
		...
		for (int i = 0; i < wci_files.length; i++) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (!(swa_dict_redis.exists(word))) {
					swa_dict_redis.set(word, ips);
				}
			}
			br.close();
			fr.close();
	...
	public String merge_sen_n(String stanf_seg_text) {
		String m_s = "";
		String[] stanf_seg_arr = stanf_seg_text.split("\\s+");
		String[] one_step_words = new String[stanf_seg_arr.length];
		int one_step_i = 0;
    ...
    public String get_direct_seg(String sw,int limit)
    {
       String seg_sw="";
           
        String m_s = "";
		String[] stanf_seg_arr = sw.split("\\s+");
		String[] one_step_words = new String[stanf_seg_arr.length];
		int one_step_i = 0;
		...
		for (int i = 0; i < (stanf_seg_arr.length); i++) {
			...
			if ((temp_words.length() > 0)
					&&(temp_words.length()<limit)&& (swa_dict_redis.exists(temp_words))) {
				one_step_words[one_step_i++] = temp_words;
				//System.out.println("two_temp_words:"+temp_words);
				i++;
			} else if ((temp_three_words.length() > 0)
					&&(temp_three_words.length()<limit)&& (swa_dict_redis.exists(temp_three_words))) {
				one_step_words[one_step_i++] = temp_three_words;
				//System.out.println("three_temp_words:"+temp_words);
				i = i + 2;
			} else if ((temp_four_words.length() > 0)
					&&(temp_four_words.length()<limit)&& (swa_dict_redis.exists(temp_four_words))) {
				one_step_words[one_step_i++] = temp_four_words;
				//System.out.println("four_temp_words:"+temp_words);
				i = i + 3;
			} else if ((temp_five_words.length() > 0)
					&&(temp_five_words.length()<limit)&& (swa_dict_redis.exists(temp_five_words))) {
				one_step_words[one_step_i++] = temp_five_words;
				//System.out.println("five_temp_words:"+temp_words);
				i = i + 4;
			} else if ((temp_six_words.length() > 0)
					&&(temp_six_words.length()<limit)&& (swa_dict_redis.exists(temp_six_words))) {
				one_step_words[one_step_i++] = temp_six_words;
				//System.out.println("six_temp_words:"+temp_words);
				i = i + 5;
			} else {
		...
		for (int i = 0; i < seven_step_words.length; i++) {
			...
			if ((!(nword.equals("")))&&(nword.length()>1)) {
				m_s = m_s + nword + " ";
			}
			// System.out.println(i + "  " + nword);
		}
	...
	public String sim_line_process(String line,int limit) {
		...
		if ((title != null) && (!title.equals("")) && (title.length() > 0)) {
			...
			try {
				...
				//System.out.println("old seg_s :"+seg_s);
				men_s = merge_sen_limit(seg_s,limit);
				men_s = men_s.trim();
				if ((men_s != null) && (!men_s.equals(""))) {
					new_line = 	men_s.trim();
				}
