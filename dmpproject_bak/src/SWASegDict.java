...
public class SWASegDict {
	...
	public String merge_sen(String stanf_seg_text) {
		...
		for (int i = 0; i < (two_step_i); i++) {
			...
			temp_three_words="";
			if (i < (two_step_i - 2)) {
				temp_three_words = two_step_words[i] + two_step_words[i + 1]
						+ two_step_words[i + 2];
				temp_three_words = temp_three_words.trim();
			}
			...
			temp_four_words="";
			if (i < (two_step_i - 3)) {
				temp_four_words = two_step_words[i] + two_step_words[i + 1]
						+ two_step_words[i + 2] + two_step_words[i + 3];
				temp_four_words = temp_four_words.trim();
			}
			
			temp_five_words="";
			if (i < (two_step_i - 4)) {
				temp_five_words = two_step_words[i] + two_step_words[i + 1]
						+ two_step_words[i + 2] + two_step_words[i + 3]
						+ two_step_words[i + 4];
				temp_five_words = temp_five_words.trim();
			}
			...
			temp_six_words="";
			if (i < (two_step_i - 5)) {
				temp_six_words = two_step_words[i] + two_step_words[i + 1]
						+ two_step_words[i + 2] + two_step_words[i + 3]
						+ two_step_words[i + 4] + two_step_words[i + 5];
				temp_six_words = temp_six_words.trim();
			}
           */
			if ((temp_words.length() > 0)
            ...
            /*
					...
					&& (swa_dict_redis.exists(temp_three_words))) {
				three_step_words[three_step_i++] = temp_three_words;
				i = i + 2;
			} else if ((temp_four_words.length() > 0)
					&& (swa_dict_redis.exists(temp_four_words))) {
				three_step_words[three_step_i++] = temp_four_words;
				i = i + 3;
			} else if ((temp_five_words.length() > 0)
					&& (swa_dict_redis.exists(temp_five_words))) {
				three_step_words[three_step_i++] = temp_five_words;
				i = i + 4;
			} else if ((temp_six_words.length() > 0)
					&& (swa_dict_redis.exists(temp_six_words))) {
				three_step_words[three_step_i++] = temp_six_words;
				i = i + 5;
			} 
			*/
			else {
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		SWASegDict swa = new SWASegDict();
		String config_file = "dict/swa_config.properties";

		swa.load_config(config_file);

		InetAddress addr = InetAddress.getLocalHost();
		String ip = addr.getHostAddress().toString();// 获得本机IP String address =
		addr.getHostName().toString();// 获得本机名

		String s1 = "正在播放《抹布女也有春天》第38集 - 国产电视剧 - 抹布女也有春天免费在线观看 - 闪播电影-最新电视剧,2013最新电影,2013快播电影,好看的电视剧免费在线观看 "; //
		// String s1="朗文英语教学实践(第4版)/杰里米·哈默(Harmer.J)-图书-亚马逊中国"; //
		// String s1="龙王令：妃卿莫属-406：孩子没爹-摘书网提供的小说免费阅读";
		String seg_s = "";
		seg_s = swa.seg(s1);
		String men_s = "";
		men_s = swa.merge_sen(seg_s);

		// String tag_s=""; // tag_s=swa.tag(men_s);
		System.out.println("seg_s:" + seg_s);
		System.out.println("men_s:" + men_s); // System.out.println("tag_s:"+tag_s);
       */
	}
}
